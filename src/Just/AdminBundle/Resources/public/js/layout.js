...
Ext.Loader.setPath('widget.just', '/bundles');
Ext.require([
    '*',
    'Portal.view.Portlet',
...
function loadtab() {
    ...
    Ext.Array.each(Ext.getCmp('contenttabpanel').items.items, function(tab, index, tabpanels) {
        if (tab.loader != null && tab.loader.url.substring(just_admin_homepage.length) == token) {
            tabfound = tab;
            return true;
        }
    });
    ...
    Ext.getCmp('contenttabpanel').add({
        ...
        listeners: {
            beforeactivate: function(tab) {
                window.location.hash = '#' + tab.loader.url.substring(just_admin_homepage.length);
                return true;
            }
