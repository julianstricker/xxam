<?php
namespace XxamMailclientBundle\Helper\WebSocketClient;

use React\EventLoop\LoopInterface;
use React\Socket\Connection;
use XxamMailclientBundle\Helper\WebSocketClient\Exception\ConnectionException;
...
class WebSocketClient
{
    ...
    private function receiveData($data, $header)
    {
        ...
        if (isset($data[0])) {
            $this->getClient()->receiveData($data);
            /*switch ($data[0]) {
                case self::MSG_WELCOME:
                    $this->getClient()->onWelcome($data);
                    break;
                case self::MSG_SUBSCRIBEDBROADCAST:
                    $this->getClient()->onSubscribedbroadcast($data);
                    break;
                case self::MSG_UNSUBSCRIBEDBROADCAST:
                    $this->getClient()->onUnsubscribedbroadcast($data);
                    break;
                case self::MSG_GETONLINE_RESPONSE:
                    $this->getClient()->onGetonlineResponse($data);
                    break;
