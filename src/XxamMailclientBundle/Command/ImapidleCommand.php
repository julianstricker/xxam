...
namespace XxamMailclientBundle\Command;
...
class ImapidleCommand extends ContainerAwareCommand {
    ...
    private function getMailaccountsForChatuser($chatuser){
        $user=$this->em->getRepository('XxamUserBundle:User')->findOneByUsername($chatuser->authid);
    ...
    protected function execute(InputInterface $input, OutputInterface $output) {
        ...
        $this->client->on('open', function (ClientSession $session) {
            ...
            // 2) publish an event
            $session->publish('com.xxam.chat.user', ['Hello, world from PHP!!!'], [], ["acknowledge" => true])->then(
                function () {
